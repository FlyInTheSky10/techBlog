...
**MSL** 是 Maximum Segment Lifetime，报文最大生存时间，超过这个时间的报文都会被丢弃。

MSL 与 TTL 的区别： MSL 的单位是时间，⽽ TTL 是经过路由跳数。

2MSL 的时间是从客户端接收到 FIN 后发送 ACK 开始计时的。如果在 TIME-WAIT 时间内，因为客户端的 ACK 没有传输到服务端，客户端⼜接收到了服务端发的 FIN 报⽂，那么 2MSL 时间将**重新计时**。

在 Linux 系统⾥ 2MSL 默认是 60 秒，那么⼀个 MSL 也就是 30 秒。Linux 系统停留在 TIME_WAIT 的时 间为固定的 60 秒

> 为什么需要 TIME_WAIT 状态？

**1. 防止旧链接的数据包**

经过 2MSL 时间，足以让两个方向所有旧链接的数据包都丢失，再出现的一定是新链接的数据包。

**2. 保证连接正常关闭**

如果 TIME_WAIT 太短，则如果客户端发出的 ACK 丢失，服务端还没有重传 FIN 客户端就进入 CLOSE 状态了，会使得服务器一直处于 LAST_ACK 状态。

客户端新的连接发送 SYN 会接收到服务端的 RST

如果 TIME_WAIT 时间够长，则：
- 服务端正确收到四次握手最后一个 ACK 报文
- 服务端没有收到四次握手最后一个 ACK 报文，重发 FIN 等待新的 ACK 报文

客户端在 TIME_WAIT 状态等待 2MSL，可以使得双方的连接可以正常关闭。
...
- window size: (win=) 表示发送方的**接受窗口大小**，使得接收方好确定**发送窗口大小**
- 发送窗口大小无法查看，若只由接收窗口决定时，可由 window size: 查看，否则如果网络影响只能估计
- TCP Window Scale: 早期 TCP 发明时网络带宽小，TCP头中接收窗口值只留下了 16bit ，无法突破 65536 大小限制。Window Scale 声明一个 Shift Count，作为2的指数乘以 TCP 接收窗口大小，即为真实 TCP 接收窗口大小

以上来自《Wireshark网络分析就这么简单》

## 流量控制

流量控制即发送方根据接收方的接收能力控制发送数据量。

### 窗口探测报文

TCP 一方收到对方的零窗口通知，就启动持续计时器，持续计时器超时则会发送窗口探测报文来获取对方的接收窗口大小，此方法可避免因对方窗口大小增大报文丢失后造成的死锁。

窗⼝探测的次数⼀般为 3 次，每次⼤约 30-60 秒（不同的实现可能会不⼀样）。如果 3 次过后接收窗⼝还是 0 的话，有的 TCP 实现就会发 RST 报文来中断连接。

## 拥塞窗口

- **慢启动**：收到 n 个 ACK，就把拥挤窗口大小增加 n 个 MSS (指数级)
- **拥塞避免**：慢启动一段时间后拥挤窗口到达一个很大的值，要缓慢一点增加，在每个往返时间增加一个 MSS (线性)
- **RTO** (Retransmission Timeout)：发出原始包到重传该包的时间
- **RTT **(Round-Trip Time)：往返时延，数据从网络一端到另一端的时间
- 重传后拥塞窗口降到 1MSS，然后再慢启动，此时过渡点 (慢启动到拥塞避免的临界点，ssthresh，**临界窗口**) 定为发生拥塞时没有被 ACK 的数据量的 1/2 (RFC 5681)，而《TCP/IP 详解》中则仅仅把临界窗口值定为上次发生拥塞的发送窗口的一半。
- **快速重传**：少量丢包时 (中间有几个包丢了)，接收方收到发送方**发送 3 个或以上的 (Dup ACK)**，则表明该包丢失，需要马上重传该包。3个 ACK 由于可能存在 TCP 包乱序到达接收方的情况，所以 3 次可以避免错判的丢包情况影响性能。
- **快速恢复**：发送快速重传后，临界窗口值设为发生拥塞时没有被 ACK 的数据量的 1/2，但是不小于 2 MSS，拥塞窗口设为临界窗口加上 3 个 MSS，继续保持在拥塞避免状态。
- **SACK** 可以增加少量丢包时的效率。ACK=z, SACK=x-y 表明 x ~ y 数据包已收到，z~x 数据包未收到，请求发送 z ~ x 数据包
- 临界窗口的大小确定：推算有多少数据包被送达 (对方 ACK 的数据包)，来评估拥塞时的网络带宽。

## Nagle 延迟确认

延迟确认未直接优化性能，只是减少确认包的发送。
...
服务端收到客户端发来的 SYN 后，内核会将连接存入半连接队列，然后向客户端发起 ACK+SYN，客户端接着返回 ACK，服务端收到这个 ACK 后将连接移出半连接队列，放入全连接队列，等待进程调用 accept() 时把连接取出来

半连接队列和全连接队列都有最大长度限制，超过限制内核可能直接**丢弃连接**或者**返回 RST 包**

- 服务端可以使用 `ss` 指令，来查看 TCP 全连接队列情况
...
- backlog 是 `listen(int sockfd, int backlog)` 函数中的 backlog 大小，Nginx 默认值是 511，可以通过修改配置⽂件设置其⻓度
