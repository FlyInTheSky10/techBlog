...
- 重传后拥塞窗口降到 1MSS，然后再慢启动，此时过渡点 (慢启动到拥塞避免的临界点，**临界窗口**) 定为发生拥塞时没有被 ACK 的数据量的 1/2 (RFC 5681)，而《TCP/IP 详解》中则仅仅把临界窗口值定为上次发生拥塞的发送窗口的一半。
- **快速重传**：少量丢包时 (中间有几个包丢了)，接收方收到发送方发送 3 个或以上的 (Dup ACK)，则表明该包丢失，需要马上重传该包。3个 ACK 由于可能存在 TCP 包乱序到达接收方的情况，所以 3 次可以避免错判的丢包情况影响性能。
